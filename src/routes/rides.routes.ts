import express from "express";
import { MeetingController } from "../controllers/meeting.controller.js";
import { RidesController } from "../controllers/rides.controller.js";
import { GeocodeController } from "../controllers/geocode.controller.js";
import asyncHandler from "express-async-handler";
import { celebrate, Segments } from "celebrate";
import { chooseRideSchema, rideSchema, rideIdsSchema, searchRideSchema, updatePickupPlanSchema } from "../models/ride.model.js";
import { idSchema } from "../models/base.model.js";

const ridesRoute = express.Router();
const base = "/ride";

//#region Geocode routes

ridesRoute.get(`${base}/geocode`, asyncHandler(GeocodeController.search));

ridesRoute.post(`${base}/meeting-point`, asyncHandler(MeetingController.calculate));

ridesRoute.post(`${base}/suggest-rides`, celebrate({ [Segments.BODY]: searchRideSchema}), asyncHandler(RidesController.suggest));

//#endregion

//#region Ride rotes

ridesRoute.post(`${base}`, celebrate({ [Segments.BODY]: rideSchema}), asyncHandler(RidesController.create));

ridesRoute.get(`${base}`, asyncHandler(RidesController.getAll));

ridesRoute.get(`${base}/:id`, celebrate({ [Segments.PARAMS]: idSchema }), asyncHandler(RidesController.getById));

ridesRoute.put(`${base}/:id`, celebrate({ [Segments.BODY]: rideSchema}), asyncHandler(RidesController.update));

ridesRoute.put(
  `${base}/:id/pickup-plan`,
  celebrate({ [Segments.PARAMS]: idSchema, [Segments.BODY]: updatePickupPlanSchema }),
  asyncHandler(RidesController.updatePickupPlan),
);

ridesRoute.get(
  `${base}/:id/meeting-point-suggestions`,
  celebrate({ [Segments.PARAMS]: idSchema }),
  asyncHandler(RidesController.suggestMeetingPoints),
);

ridesRoute.get(
  `${base}/:id/pickup-context`,
  celebrate({ [Segments.PARAMS]: idSchema }),
  asyncHandler(RidesController.getPickupContext),
);

ridesRoute.put(
  `${base}/:rideId/choose/:userId`,
  celebrate({ [Segments.PARAMS]: rideIdsSchema, [Segments.BODY]: chooseRideSchema }),
  asyncHandler(RidesController.chooseRide),
);

ridesRoute.put(`${base}/:rideId/calcel-passenger/:userId`, celebrate({ [Segments.PARAMS]: rideIdsSchema }), asyncHandler(RidesController.cancelPassengerRide));

ridesRoute.put(`${base}/:rideId/calcel-driver/:userId`, celebrate({ [Segments.PARAMS]: rideIdsSchema }), asyncHandler(RidesController.cancelDriverRide));

//#endregion

export default ridesRoute;
